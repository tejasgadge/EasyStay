...
export default function PlaceGallery({place}) {
  ...
  if (showAllPhotos) {
    return (
      <div className="absolute inset-0 bg-black text-white min-h-screen">
        <div className="bg-black p-8 grid gap-4 items-center">
          <div>
          ...
          <div className="flex  flex-col gap-8 justify-center max-w-full  items-center ">
          {place?.photos?.length > 0 && place.photos.map(photo => (
            <div>
              <img className="px-12 rounded-2xl max-w-3xl" src={'http://localhost:4000/uploads/'+photo} alt=""/>
            </div>
          ))}
          </div>
        </div>
  ...
  return (
    <div className="relative">
      <div className="grid gap-2 grid-cols-[2fr_1fr_1fr] rounded-3xl overflow-hidden max-h-full">
        <div>
          {place.photos?.[0] && (
            <div>
